...
train_df_xba = pd.read_sql("""
    ...
    WHERE s.launch_speed IS NOT NULL AND s.launch_angle IS NOT NULL
""", conn)
...
xba_df = raw_df[[
    'events','hit_distance_sc','launch_speed','launch_angle','game_date',
    'home_team','away_team','inning_topbot','game_pk','batter','pitcher',
...
# Drop duplicate AB_id in dataframe
xba_df = xba_df.drop_duplicates(subset=['AB_id'])
...
data_xba = [
    ...
    for row in xba_df[[
        ...
        'batter','pitcher','park_hit_rate','xBA','AB_id'
...
print(f"✅ xBA uploaded: {len(data_xba)} rows")
...
train_df_xruns = pd.read_sql("""
    SELECT hit_distance_sc, launch_angle, launch_speed, outs_when_up,
           runners_on_base, runs_scored, venue_id
...
features_xruns = ['hit_distance_sc','launch_angle','launch_speed','outs_when_up','runners_on_base','park_run_factor']
...
venue_lookup = pd.read_sql("SELECT DISTINCT game_pk, venue_id FROM statcast_data", conn)
xruns_df = xruns_df.merge(venue_lookup, on='game_pk', how='left')
...
xruns_df = xruns_df.drop_duplicates(subset=['AB_id'])

data_xruns = [
    tuple(None if pd.isna(x) else x for x in row)
    for row in xruns_df[[
        'unique_game_id','game_pk','game_id','game_date','home_team','away_team',
        'inning_topbot','team_batting','team_pitching','batter','pitcher',
        'hit_distance_sc','launch_angle','launch_speed','outs_when_up','runners_on_base',
        'runs_scored','expected_runs','events','description','AB_id','inning','park_run_factor'
    ]].itertuples(index=False, name=None)
]
...
for i in tqdm(range(0,len(data_xruns),5000), desc="Uploading xRuns"):
    cursor.executemany(insert_query_xruns, data_xruns[i:i+5000])
    conn.commit()

print(f"✅ xRuns uploaded: {len(data_xruns)} rows")

# ==========================================
# MODULE 3: Game Scores
# ==========================================
TEAM_ABBR_MAP = {
...
def get_mlb_schedule(start_date,end_date):
    url = f"https://statsapi.mlb.com/api/v1/schedule?sportId=1&startDate={start_date}&endDate={end_date}&hydrate=linescore,probablePitcher"
    ...
    for date_data in data.get("dates",[]):
        for game in date_data.get("games",[]):
            if game.get("gameType") in ["E"]: continue
...
def run_game_score_update():
    ...
    data_values = [(
        r['unique_game_id'],r["game_pk"],r["game_date"],r["game_time"],r["game_status"],
        r["home_team"],r["away_team"],r["home_pitcher"],r["away_pitcher"],        r["home_score_final"], r["away_score_final"], r["current_inning"], r["inning_state"]
    ...
    for i in tqdm(range(0, len(data_values), 1000), desc="Uploading Game Scores"):
...
cursor.execute("""
SELECT DISTINCT batter FROM expected_runs_2025
UNION
SELECT DISTINCT pitcher FROM expected_runs_2025
""")
...
"""
cursor.executemany(insert_query_names, data_names)
conn.commit()
cursor.close()
conn.close()
