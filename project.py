...
cursor.execute("""CREATE TABLE statcast_2025 (id INT AUTO_INCREMENT PRIMARY KEY, events VARCHAR(255), hit_distance_sc FLOAT, launch_speed FLOAT, launch_angle FLOAT, game_date DATE, team_batting VARCHAR(255), team_pitching VARCHAR(255), gameid VARCHAR(255), gamepk BIGINT, player_id BIGINT, pitcher_id BIGINT, xBA FLOAT, AB_id VARCHAR(255))""")
...
for i in tqdm(range(0, len(data_xba), 5000), desc="Uploading xBA"):
    cursor.executemany("INSERT INTO statcast_2025 (events, hit_distance_sc, launch_speed, launch_angle, game_date, team_batting, team_pitching, gameid, gamepk, player_id, pitcher_id, xBA, AB_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", data_xba[i:i+5000])
    conn.commit()
...
for i in tqdm(range(0, len(data_xruns), 5000), desc="Uploading xRuns"):
    cursor.executemany("INSERT INTO expected_runs_2025 (game_pk, game_id, game_date, home_team, away_team, inning_topbot, team_batting, team_pitching, batter, pitcher, hit_distance_sc, launch_angle, launch_speed, outs_when_up, runners_on_base, runs_scored, expected_runs, events, description, AB_id, inning) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", data_xruns[i:i+5000])
    conn.commit()
...
def run_game_score_update():
    ...
    try:
        ...
        cursor.execute("DELETE FROM actual_game_scores_2025")
        
        insert_query = """
            INSERT INTO actual_game_scores_2025 (
